function [RGBcalib, W, bIndBd] = cameraCalibrate(RGB,Aperture,Shutter,whichCamera,transformType,bitsOut,whichMeasure,bComputeTransform,bPLOT)

% function [RGBcalib W bIndBd ] = cameraCalibrate(RGB,Aperture,Shutter,whichCamera,transformType,bitsOut,whichMeasure,bComputeTransform,bPLOT)
%
%   example call: [Ilms] = cameraCalibrate(I,Aperture,Shutter,'D7H','LMS',16,2,0,0); % transform to 16-bit LMS cone  responses
%                 [Ixyz] = cameraCalibrate(I,      [],     [],'D7H','XYZ',16,2,0,0); % transform to 16-bit XYZ color space
%                 [Ipht] = cameraCalibrate(I,      [],     [],'D7H','PHT',16,2,0,0); % transform to 16-bit photopic luminance
%                 [Im1]  = cameraCalibrate(I,      [],     [],'D7H', 'M1', 8,2,0,0); % transform to  8-bit phosphor spectra
%                                                                                                     outputs for three monitor rig in GeislerLab
% convert raw RGB image to XYZ, sRGB, LMS, PHT, etc.
%
...
if bitsOut > 16 || bitsOut < 1
    error(['cameraCalibrate: WARNING! bitsOut outside handled range of [1 16]. bitsOut = ' num2str(bitsOut)]);
end
if rem(bitsOut,1) ~= 0
    error(['cameraCalibrate: WARNING! bitsOut must be integer valued. bitsOut = ' num2str(bitsOut)]);
end
...
if strcmp(transformType,'M1') || strcmp(transformType,'M2') || strcmp(transformType,'MF')
    ...
    disp(['cameraCalibrate: WARNING! ' transformType ' brightnessLevel hard-coded to ' num2str(brightnessLevel)]);
...
% SCALE IMAGE
% [RGBcalib,bIndBd] = imageRescale(RGBcalib,quantile(RGBcalib(:),[.001 .999]),[0 2^bitsOut-1],0,0);   
[RGBcalib,bIndBd] = imageRescale(RGBcalib,quantile(RGBcalib(:),[.0002 .9998]),[0 2^bitsOut-1],0,0);   

% RETURN INTEGER VALUES WITH bitsOut BIT DEPTH
RGBcalib = floor(RGBcalib);

% SET NEGATIVE VALUES TO VALUE VANISHINGLY CLOSE TO ZERO
if ~exist('indNeg','var')
...
if strcmp(whichCamera,'D7H')
    ...
    elseif strcmp(transformType,'VRP') 
           W = [0.9191    0.2601    0.0095
                0.0492    1.1295    0.1899
                0.0279    0.1442    0.8458]; 
           K = [3.5459   1.6599    1.9672].*1.0e7;
